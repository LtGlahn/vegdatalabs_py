...
class Objekt:
    ...
    def lokasjon( self):
        return self.data['lokasjon']

    def omraadenavn(self, omradetype):
...
class Resultat:
    ...
    def __init__(self, data):
        ...
        self.vegObjekter  = []
        for k in self.data['resultater'][0]['vegObjekter']:
            self.vegObjekter.append( Objekt( k) )
...
def query_search(objekttyper, lokasjon='', params = {}):
    """Leverer en instans av klassen Resultat etter en spørring
    mot NVDB APIets søkegrensesnitt:
    https://www.vegvesen.no/nvdb/api/dokumentasjon/sok/

    Argumenter:
    objekttyper -- Påkrevd: Liste ned objekttyper
    lokasjon    -- Frivillig: Dictionary med lokasjoner (default: '')
    params      -- Frivillig: Dictionary med øvrige nøkkeord-parametre som
                sendes til NVDB api'ets søkefunksjon.
    ...
    params['kriterie'] = json.dumps(sokobjekt)

    return Resultat(query('/sok', params ))
